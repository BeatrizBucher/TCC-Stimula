...
function MainEditarCliente() {
    ...
    async function getCliente() {
        ...
        try {
            let consulta = await axios.get(`http://localhost:3001/api/listarClientePorId/${id}`,{
                headers: { 'Authorization': `Bearer ${token}` }
            });
           
            setNome(consulta.data.nome)
            setSobrenome(consulta.data.sobrenome)
            setCep(consulta.data.cep)
            setLogradouro(consulta.data.logradouro)
            setComplemento(consulta.data.complemento)
            setCidade(consulta.data.cidade)
            setUf(consulta.data.uf)
            setIdade(consulta.data.idade)
            setCpf(consulta.data.cpf)
            setEmail(consulta.data.email)
            setTelefone(consulta.data.telefone)
            setGenero(consulta.data.genero)
            setObservacao(consulta.data.observacao)
            notificacaoSucesso()
        } catch (erro) {
