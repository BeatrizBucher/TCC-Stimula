...
function MainEditarContrato() {
  ...
  async function getContrato() {
    ...
    try {
      ...
      console.log(consulta.data.plano_id);

      setCod(consulta.data.cliente_id || '')
      setServicos(consulta.data.servico_id)
      setPlanos(consulta.data.plano_id)
      setPagamento(consulta.data.pagamento_id)
    } catch (erro) {
  ...
  async function editarContrato(event) {
    ...
    try {
      let cadastro = await axios.put(`http://localhost:3001/api/atualizarContrato/${id}`, contrato, {
          headers: {                    
